 ...
 * WDT measures light conditions and battery voltage every 8 seconds    -- in
 * onWDT()
 *  --> If dark and full, switch WDT to 0.5 s interrupts
 * If less than 4 Blinky sequences are in progress:
 *      There is a certain change that WDT starts a new Blinky sequence -- in
 * onWDT() Blinky
 *  - int8_t pwmChannel                 //Set to a random and FREE LED channel,
 * -1 = inactive Blinky slot
 *  - uint8_t *glowingPatternPointer    //Set to a random pattern with
 * preference for more dim patterns
 *  - uint8_t samplesLeftInPattern      //How many samples left to show in this
 * pattern
 *  - uint8_t blinkySpeed               //Delay between samples, set to a random
 * value with reasonable limits Blinky is updated and shown on the LEDs by
 * setting curPWMvalues[]    -- in onNewFrame() if samplesLeftInPattern==0:
 * pwmChannel=-1   --> Blinky slot is inactive
 *
 * Things to do
 * --------------
 * All pwm values are iterated during every wakeup in the PWM routine which is
 * rather inefficient (even if the OCR register is used for precise wakeups)
 *
 * Better: 8-10 Wakeups per PWM cycle, arranged in a 2^N way
 *
 * Each wakeup uses precalculated port settings which are output from ram to the
 * port pins For 10 bit PWM we have the arrays uint8_t pwmPrecalcPortB[10],
 * pwmPrecalcPortC[10], pwmPrecalcPortD[10]
 *
 * there is a setPwmValue( uint8_t channel, uint16_t value ) routine, which does
 * the precalculation from main()
 *
 * it chooses a pwmPrecalcPortX[] array and a pinNumber, depending on the
 * channel. Then it does: uint8_t *precalc = pwmPrecalcPortX; for( uint8_t
 * pwmStep=0; pwmStep<10; pwmStep++ ){ if( IBI( value, pwmStep ) ){ SBI(
 * *precalc, pinNumber ); } else { CBI( *precalc, pinNumber );
 *      }
 *      precalc++;
 * }
 *
...
void init(void) {
  ...
  //--------------------------------------------------
  // Serial debugging at 115200 baud/s
  //--------------------------------------------------
  ...
  //    WDTCSR  = (1 << WDIE) | (1 << WDP3) | (1 << WDP0); //Set prescaler to
  //    128 kHz / 1024k = 8s, interrupt mode WDTCSR  = (1 << WDIE) | (0 << WDP3)
  //    | (1 << WDP2) | (1 << WDP1) | (0 << WDP0);  //Set prescaler to 1s,
  //    interrupt mode
  WDT_SLOW_MODE();
  //------------------------------------------------------
  // init PRNG, load last seed from EEPROM, calculate next seed and store back
  // to EEPROM, use another polynom as in lfsr()
  //------------------------------------------------------
...
int main() {
  init(); // Now Timer0 is running and will generate an interrupt on its first
          // overflow
  rprintf("\n\n---------------------------------------\n");
  ...
  while (1) { // We woke up. Figure out what to do (Wake-up sources: T0 compare
              // match, WDT overflow)
    if (IBI(flags, FLAG_wakeWDT)) { // This is called every PWM_CYCLES_PER_FRAME
      onWDT(); // Update the PWM values, switch OFF T0 to save energy if
               // finished to glow (Then we have less wake up interrupts and can
               // do deep power down)
...
void houseKeeping() {
  ...
  if (vCap < LBATT_THRESHOLD) { // Go comatose!
    rprintf(
        "currentState = STATE_LOW_BATT  (Battery undervoltage!)  zzZZzzZZ\n");
...
//  Glowing:  start T0 PWM interrupt and switch to "Idle" sleep mode (Only CPU
//  core is off during sleep) Standby:  stop  T0 PWM interrupt and switch to
...
void onWDT() {
  ...
  if (currentState ==
    ...
    if (wdtWakeupCounter > WDT_N_WK_ADC_LBATT) {
      ...
      wdtWakeupCounter = 0;
      vCap = readVCAP();
      if (vCap > LBATT_THRESHOLD + 250) { // We have charged a bit, wake up a
                                          // bit
...
// Internal readADC routine, does n ADC readings (10 bit each) and returns the
// sum result
uint16_t readADC(uint8_t mux, uint8_t n) {
  ...
  if (!IBI(flags, FLAG_PWM_IS_ON)) { // Dont change sleep mode if T1 is running
                                     // (we would deactivate it)
    SLEEP_SET_ADC();
  }
  for (i = 0; i < n; i++) {
    SLEEP();
  ...
  //  SBI( TIMSK0, OCIE0A );                                      //Switch timer
  //  interrupt back on
  return result;
}
...
ISR(PCINT0_vect) {
  ...
  if (IBI(status, RX_DR)) { // RX data received
    ...
    //      procedure for handling this interrupt should be: 1) read payload
    //      through SPI, 2) clear RX_DR IRQ, 3) read FIFO_STATUS to check if
    //      there are more payloads available in RX FIFO, 4) if there are more
    //      data in RX FIFO, repeat from step 1)
  ...
  if (IBI(status, MAX_RT)) { // Maximum number of TX retries reached
    ...
    // if( nRfIsTxFifoFull() ){        //Okay screw this, enough playing around
    // will empty the FIFO
    //     rprintf("FIFO_CLEAR\n");
    nRfFlush_tx();
    // }
