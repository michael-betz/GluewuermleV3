...
void init(void) {
	...
	//    128 kHz / 1024k = 8s, interrupt mode WDTCSR  = (1 << WDIE) | (0 <<
	//    WDP3) | (1 << WDP2) | (1 << WDP1) | (0 << WDP0);  //Set prescaler to
	//    1s, interrupt mode
	WDT_SLOW_MODE();
	//------------------------------------------------------
...
void houseKeeping() {
	...
	if (vCap < LBATT_THRESHOLD) { // Go comatose!
		...
		rprintf(
			"currentState = STATE_HIGH_BATT  (Battery overvoltage!)  BUURN!\n");
...
void onWDT() {
	...
	if (currentState == STATE_LOW_BATT) {
		...
		if (wdtWakeupCounter > WDT_N_WK_ADC_LBATT) {
			...
			wdtWakeupCounter = 0;
			vCap = readVCAP();
			if (vCap > LBATT_THRESHOLD + 250) { // We have charged a bit, wake
												// up a bit
	...
	case STATE_GLOW_FULLSCREEN:
		...
		if (wdtWakeupCounter == 0) { // Triggered every 32 s if glowing
			...
			if (temp <= 12) { // Get out of Fullscreen glowing mode
				rprintf(
					"currentState = STATE_GLOW_GLUEH  (Enough Fullscreen)\n");
				currentState = STATE_GLOW_GLUEH;
...
ISR(PCINT0_vect) {
	...
	if (IBI(status, RX_DR)) {
		...
		//      there are more payloads available in RX FIFO, 4) if there are
	...
	if (IBI(status, MAX_RT)) {
		...
		// if( nRfIsTxFifoFull() ){        //Okay screw this, enough playing
		// around will empty the FIFO
		//     rprintf("FIFO_CLEAR\n");
		nRfFlush_tx();
		// }
	}
