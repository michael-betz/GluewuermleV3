...
void houseKeepingWhileGlowing() {
  ...
  if (currentState == STATE_GLOW_GLUEH) {
    ...
    temp_32 =
        ((int32_t)(vCap - MINIMUM_VOLTAGE)) *
        g_maximumCoffeeLevel; // temp2 = 0 ... (MAXIMUM_VOLTAGE-MINIMUM_VOLTAGE)
...
oneBug_t *startSeqGlueh(uint8_t bugId) {
  ...
  if (patternID <
    ...
    //      bug[3]-=en/4, bug[4]-=en/8, etc. overall energy billiance is reduced
    //      by en
  ...
  for (nled = 0; nled < NLEDS; nled++) {
    ...
    case BUG_INACTIVE_RESCHEDULE:
      if (bug->availableEnergy > 0) {
        startSeqGlueh(
            nled); // Starts a new sequence (with a rather small propability)
      }
    }
    bug++;
  }
    ...
    case ROTATE:
      ...
      if (tempFrameCounter++ >= 2) {
        ...
        for (temp = 0; temp < MAX_ELEMENTS - 1; temp++) {
          curPWMvalues[temp] =
              curPWMvalues[temp + 1]; // Move all elements one step down
        }
