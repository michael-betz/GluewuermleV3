...
void nRfInit() { // Init with default config
	...
	nRfWrite_register(
		FEATURE,
		0b00000111); // Enable features: Dynamic payload length, Ack packets
					 // with payload, Disabled Ack for some packets
	nRfFlush_rx();
	nRfFlush_tx();
...
uint8_t nRfIsRxDataReady() {
	...
	//  interrupt flag
	return (!(nRfRead_register(FIFO_STATUS) &
			  (1 << RX_EMPTY))); // Check FIFO status
}
