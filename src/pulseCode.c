...
volatile uint16_t pwmPortRaw[NBITS]; // N Precalculated Pin values for 16 ports
                                     // to achieve N bit pulse code modulation
// we start this array at bit6! the lower bits are handled by the ASM routine
uint16_t delayValues[] = {2 * 64 - 5 - 36, 2 * 128,  2 * 256,
                          2 * 512,         2 * 1024, 2 * 2048,
                          2 * 4096,        2 * 8192, 2 * 16384L};
// !!! NEVER EVER Access the following global variables from main, they are only
// for the ISR !!!
volatile uint16_t *portDataPointer =
    pwmPortRaw; // 16 bit pointer to portData entry of interest
volatile uint16_t *portDelayPointer =
    delayValues; // 16 bit pointer to next DELAY value
