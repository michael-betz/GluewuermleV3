...
static uint8_t nRfRead_transaction( uint8_t command, uint8_t *readData, uint8_t len ){
    uint8_t status;
    NRF_CHIP_SELECT();
...
static uint8_t nRfWrite_transaction( uint8_t command, const uint8_t *writeData, uint8_t len ){
    uint8_t status;
    NRF_CHIP_SELECT();
    status = spiTXRXbyte( command );
    while ( len-- ){
